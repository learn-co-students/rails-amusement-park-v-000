   its because you have the root set to 'user#index' AND 
# in your users_controller you have a before_action :authenticate user. 
# So any time you try to go to any user route, index, show, new, 
# it will try to authenticate the user. 
# If the user is not logged in, then you have them redirected to the root path, 
# which is a user route, so then it tries to authenticate again, and again.....
